...
# Encrypt to cipher
def encryption(plain_str, original_key):
...
# Decrypt cipher
def decryption(enc, original_key):
...
# Start
# Encryption
def encrypt_str(plain_str, original_key):
    enc = ''
    sub_str = [(plain_str[i:i+8]) for i in range(0, len(plain_str), 8)]
    for sub in sub_str:
        # print("Plain sub text:", sub)
        enc += encryption(sub, original_key)

    # print("Final cipher text:", enc)
    return enc

# Decyption
def decrypt_str(enc, original_key):
    dec = ''

    # we'll call the decryption function
    sub_str = [(enc[i:i+8]) for i in range(0, len(enc), 8)]
    for sub in sub_str:
        dec += decryption(sub, original_key)

    # print("Decryption of Cipher :", dec)
    return dec

if __name__ == '__main__':
    enc = encrypt_str("Lorem ipsum dolor sit amet, consectetur adipiscing elit.", "17CJones")
    print("Enc:", enc)
    dec = decrypt_str(enc, "17CJones")
    print("Dec:", dec)